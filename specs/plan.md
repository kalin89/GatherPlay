...
- **AiContentModule**: expone métodos como `getTriviaQuestions(categoria)`, `getSongForRocola()`, `getWordChainCategory()`. Se llama **antes** de iniciar una ronda (para tener el contenido listo), nunca durante el conteo del temporizador — así una llamada lenta a IA nunca bloquea el juego en vivo.
